...
TRPO Working: 
    1. Current Policy Evaluation: Evaluating the current policy (a strategy for selecting actions) using the environment.
    
    2. Policy Improvement: It then tries to improve the policy by adjusting its parameters to increase the expected reward. 
        This is done by taking small steps in the direction that improves the policy.
        
    3. Trust Region: TRPO includes a "trust region," which limits how much the policy parameters can change at each step. 
        This helps ensure that the policy changes are not too drastic, which can lead to unstable learning.
        
    4. Constraint Optimization: Form of constrained optimization to ensure that the new policy is not too different from the old policy. Maintain stability during training.
    
    5. Iterative Process: TRPO iteratively repeats the evaluation, improvement, and constraint optimization steps to gradually improve the policy over time.
"""
import torch
import gym
...
class TRPOAgent:
    ...
    def __init__(self, policy, discount=0.98, kl_delta=0.01, cg_iteration=10,
        ...
        discount : float, optional
            Discount factor for future rewards, by default 0.98.
        kl_delta : float, optional
            KL divergence threshold, by default 0.01.
        cg_iteration : int, optional
            Number of conjugate gradient iterations, by default 10.
        cg_dampening : float, optional
            Dampening factor for conjugate gradient, by default 0.001.
        cg_tolerance : float, optional
            Tolerance for conjugate gradient convergence, by default 1e-10.
        cg_state_percent : float, optional
            Percentage of states to use for conjugate gradient, by default 0.1.
        """
        self.policy = policy
